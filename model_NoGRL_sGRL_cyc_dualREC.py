...
def create_model(inputsX, inputsY, a):
    ...
    with tf.name_scope("code_recon_trainX2Y"):
        ...
        code_reconX2Y_grads_and_vars = code_reconX2Y_optim.compute_gradients(code_reconX2Y_loss, var_list=code_reconX2Y_tvars)
    ...
    with tf.name_scope("code_recon_trainY2X"):
        code_reconY2X_tvars = [var for var in tf.trainable_variables() if
                            var.name.startswith("generatorY2X") or
                            var.name.startswith("generatorX2Y_encoder")]
        code_reconY2X_optim = tf.train.AdamOptimizer(a.lr, a.beta1)
        code_reconY2X_grads_and_vars = code_reconY2X_optim.compute_gradients(code_reconY2X_loss, var_list=code_reconY2X_tvars)
    ...
    return Model(
        ...
        train=tf.group(update_losses, incr_global_step, genX2Y_train,
                       genY2X_train, autoencoderX_train, autoencoderY_train,code_reconX2Y_train,
                       code_reconY2X_train,discrim_sharedX2Y_train,discrim_sharedY2X_train,
