...
from discriminatorWGANGP_unpaired import *
...
Model = collections.namedtuple("Model", "outputsX2Y, outputsY2X,\
                               ...
                               code_reconX2Y_loss, code_reconY2X_loss\
...
def create_model(inputsX, inputsY, a):
    ...
    with tf.name_scope("autoencoderX"):
        with tf.variable_scope("generatorY2X_decoder", reuse=True):
            out_channels = int(inputsX.get_shape()[-1])
            auto_outputX = create_generator_decoder(sR_X2Y, eR_X2Y, out_channels, a)

    with tf.name_scope("autoencoderY"):
        with tf.variable_scope("generatorX2Y_decoder", reuse=True):
            out_channels = int(inputsY.get_shape()[-1])
            auto_outputY = create_generator_decoder(sR_Y2X, eR_Y2X, out_channels, a)
    ...
    with tf.name_scope("discriminatorX2Y_loss"):
        ...
        interpolates = alpha * tf.reshape(outputsX2Y,[-1,OUTPUT_DIM])
        with tf.variable_scope("discriminatorX2Y", reuse=True):
            gradients = tf.gradients(create_discriminator_up(tf.reshape(interpolates,[-1,IMAGE_SIZE,IMAGE_SIZE,3]),a),
                         [interpolates])[0]
    ...
    with tf.name_scope("discriminatorY2X_loss"):
        ...
        interpolates = alpha * tf.reshape(outputsY2X,[-1,OUTPUT_DIM])
        with tf.variable_scope("discriminatorY2X", reuse=True):
            gradients = tf.gradients(create_discriminator_up(tf.reshape(interpolates,[-1,IMAGE_SIZE,IMAGE_SIZE,3]),a),
                         [interpolates])[0]
        slopes = tf.sqrt(tf.reduce_sum(tf.square(gradients),
    ...
    with tf.name_scope("code_reconX2Y_loss"):
        ...
        code_reconX2Y_loss = a.l1_weight * (code_sR_X2Y_recon_loss + code_eR_X2Y_recon_loss)

    with tf.name_scope("code_reconY2X_loss"):
        ...
        code_reconY2X_loss = a.l1_weight * (code_sR_Y2X_recon_loss + code_eR_Y2X_recon_loss)
    ...
    with tf.name_scope("code_recon_trainX2Y"):
        code_reconX2Y_tvars = [var for var in tf.trainable_variables() if
                               var.name.startswith("generatorX2Y_decoder") or
                               var.name.startswith("generatorY2X_encoder")]
        code_reconX2Y_optim = tf.train.AdamOptimizer(a.lr, a.beta1)
        code_reconX2Y_grads_and_vars = code_reconX2Y_optim.compute_gradients(code_reconX2Y_loss,
                                                                             var_list=code_reconX2Y_tvars)
        code_reconX2Y_train = code_reconX2Y_optim.apply_gradients(code_reconX2Y_grads_and_vars)

    with tf.name_scope("code_recon_trainY2X"):
        code_reconY2X_tvars = [var for var in tf.trainable_variables() if
                               var.name.startswith("generatorY2X_decoder") or
                               var.name.startswith("generatorX2Y_encoder")]
        code_reconY2X_optim = tf.train.AdamOptimizer(a.lr, a.beta1)
        code_reconY2X_grads_and_vars = code_reconY2X_optim.compute_gradients(code_reconY2X_loss,
                                                                             var_list=code_reconY2X_tvars)
        code_reconY2X_train = code_reconY2X_optim.apply_gradients(code_reconY2X_grads_and_vars)
    ...
    return Model(
        ...
        code_reconX2Y_loss=ema.average(code_reconX2Y_loss),
        code_reconY2X_loss=ema.average(code_reconY2X_loss),
        ...
        train=tf.group(update_losses, incr_global_step, genX2Y_train,
                       ...
                       code_reconY2X_train, code_reconX2Y_train,
